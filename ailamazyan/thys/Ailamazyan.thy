...
lemma ad_agr_list_rev_mono: "Y \<subseteq> X \<Longrightarrow> ad_agr_list Y ys xs \<Longrightarrow>
  Inl -` set xs \<subseteq> Y \<Longrightarrow> Inl -` set ys \<subseteq> Y \<Longrightarrow> ad_agr_list X ys xs"
...
proof (induction ts)
  ...
  proof (cases t)
    ...
    have fv_t: "fv_fo_term_set t = {}"
      by (auto simp: Const)
    have "\<And>t'. t' \<in> set ts \<Longrightarrow> sp_equiv_pair (\<sigma> \<cdot>e t, \<tau> \<cdot>e t) (\<sigma> \<cdot>e t', \<tau> \<cdot>e t')"
      subgoal for t'
        apply (cases t')
...
lemma fo_nmlzd_take: "fo_nmlzd AD xs \<Longrightarrow> fo_nmlzd AD (take i xs)"
...
lemma ad_agr_map: "length xs = length ys \<Longrightarrow> inj_on m (dom m) \<Longrightarrow>
  ...
  | Inr n \<Rightarrow> m x = Some y \<and> (case y of Inl z \<Rightarrow> z \<notin> AD | Inr _ \<Rightarrow> True))) \<Longrightarrow>
...
proof -
  ...
  have "ad_agr_list AD (map \<sigma> ns) (map (Inl \<circ> \<tau>) ns)"
    apply (auto simp: ad_agr_list_def ad_equiv_list_def)
    subgoal for a b
      using f_def(2)
      by (auto simp: set_zip \<tau>_def ad_equiv_pair.simps split: sum.splits)+
    using f_def(1) f_out
    apply (auto simp: sp_equiv_list_def pairwise_def set_zip \<tau>_def inj_def split: sum.splits)+
    done
  then show ?thesis
...
lemma norm_proj_fmla_esat_sat:
  ...
  subgoal for \<sigma>
    ...
    apply auto
    subgoal for \<tau>
      by (auto intro!: bexI[of _ "map (Inl \<circ> \<tau>) (fv_fo_fmla_list \<phi>)"] fo_nmlz_eqI)
...
proof -
  ...
  proof (rule set_eqI, rule iffI)
    ...
    moreover have "\<And>n n'. t = Var n \<Longrightarrow> t' = Var n' \<Longrightarrow> n \<noteq> n' \<Longrightarrow>
      ...
      apply (auto simp: \<phi>_def AD_def pes_def proj_fmla_map fo_nmlz_Cons fv_fo_fmla_list_def
          split: sum.splits)

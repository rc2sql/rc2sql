...
let sigusr2_handler =
  Sys.Signal_handle (fun _ -> Misc.usr2 := true)

let sigalrm_handler =
  Sys.Signal_handle (fun _ -> Misc.alrm := true)
...
let usage_string = 
  "Usage: monpoly-reg -sig <file> -formula <file> [-negate] [-log <file>]
               [-help] [-version] [-debug <unit>] [-verbose] 
               [-check] [-sigout] [-mem] [-mem2] [-nonewlastts] [-parselog]
...
let main () = 
  ...
  if !displayver then
    print_endline ("MonPoly-Reg, version " ^ version)
  else if !sigfile = "" then
    ...
    begin
      ...
      if !parselogopt then
	Algorithm.parse_log sign !logfile 
...
let _ = 
  Arg.parse [
    ...
    "-mem2", Arg.Set memarg2, "\t\tShow memory usage on stderr (variant)";
    "-nonewlastts", Arg.Clear Misc.new_last_ts, "\tDo not add a last maximal time-stamp";
    "-parselog", Arg.Set parselogopt, "\tParse the log without monitoring";
    ...
    "-mona_dir", Arg.Set_string Dfa_store.mona_dir, "\tSet directory with Mona automata";
  ]
    (fun _ -> ())
    usage_string;
  main ();

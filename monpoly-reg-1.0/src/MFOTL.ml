...
let ts_of_string err_place str = 
  ...
  with Failure "int_of_string" -> 
    let msg = Printf.sprintf 
      "[%s, MFOTL.ts_of_string] Cannot convert %s into a timestamp" 
      err_place str 
    in
    failwith msg
...
let cst_of_ts t = Str (string_of_int t)
...
let ts_invalid = -1
let ts_null = 0
let ts_max = 1 lsl 31 (* because (normally) C integers are of 4 bytes *)
...
let print_interval (a,b) = 
  (match a with
    | OBnd a -> Printf.printf "(%d," a
    | CBnd a -> Printf.printf "[%d," a
    | Inf -> Printf.printf "(*,");
  match b with
    | OBnd b -> Printf.printf "%d) " b
    | CBnd b -> Printf.printf "%d] " b
    | Inf -> Printf.printf "*) " 

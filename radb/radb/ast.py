...
class Context:
    def __init__(self, configured, db, check, views, cmdargs, cse):
...
class StatementContext(Context):
    def __init__(self, context: Context, root: 'RelType'):
        super(StatementContext, self).\
            __init__(context.configured, context.db, context.check, context.views, context.cmdargs, context.cse)
        self.root = root
...
class RelExpr(Node):
    ...
    def execute(self, context):
        blocks = ['rat{}({}) AS ({})'.format(idx, ', '.join(['a{}'.format(i) for i in range(n)]), query) for query, (idx, n) in context.cse.items()]
        assert(len(blocks) > 0)
        query = 'WITH ' + ',\n     '.join(blocks)
...
class RelRef(RelExpr):
    ...
    def validateSubtree(self, context: StatementContext):
        ...
        if context.db.table_exists(self.rel):
            query = 'SELECT * FROM {}'.format(self.rel)
            attrspecs = [AttrSpec(self.rel, attr, type)\
                         for attr, type in context.db.describe(self.rel)]
            if query in context.cse.keys():
              idx, _ = context.cse[query]
              self.type = RelType(idx, attrspecs)
              self.view = None
            else:
              idx = len(context.cse)
              self.type = RelType(idx, attrspecs)
              context.cse[query] = (idx, len(attrspecs))
              self.view = None
            return
...
class Project(RelExpr):
    ...
    def validateSubtree(self, context: StatementContext):
        ...
        if query in context.cse.keys():
          idx, _ = context.cse[query]
          self.type = RelType(idx, output_attrspecs)
        else:
          idx = len(context.cse)
          self.type = RelType(idx, output_attrspecs)
          context.cse[query] = (idx, len(output_attrspecs))
    def info(self):
...
class Select(RelExpr):
    ...
    def validateSubtree(self, context: StatementContext):
        ...
        if query in context.cse.keys():
          idx, _ = context.cse[query]
          self.type = RelType(idx, self.inputs[0].type.attrs)
        else:
          idx = len(context.cse)
          self.type = RelType(idx, self.inputs[0].type.attrs)
          context.cse[query] = (idx, len(self.inputs[0].type.attrs))
    def info(self):
...
class Join(RelExpr):
    ...
    def validateSubtree(self, context: StatementContext):
        ...
        if self.cond is None:
            ...
            if query in context.cse.keys():
              idx, _ = context.cse[query]
              self.type = RelType(idx,
                                  self.inputs[0].type.attrs + self.inputs[1].type.attrs)
            else:
              idx = len(context.cse)
              self.type = RelType(idx,
                                  self.inputs[0].type.attrs + self.inputs[1].type.attrs)
              context.cse[query] = (idx, len(self.inputs[0].type.attrs + self.inputs[1].type.attrs))
    def info(self):
    ...
    def query(self):
        ...
        if self.inputs[0].type.sql_rel() == self.inputs[1].type.sql_rel():
          from_clause = self.inputs[0].type.sql_rel()
        else:
          from_clause = "{}, {}".format(self.inputs[0].type.sql_rel(), self.inputs[1].type.sql_rel())
        return 'SELECT {} FROM {}{}'\
               .format(select, from_clause, where)
...
class Cross(RelExpr):
    ...
    def query(self):
        right = 'tmp'
        return 'SELECT * FROM {}, {} AS {}'\
               .format(self.inputs[0].type.sql_rel(), self.inputs[1].type.sql_rel(), right)
...
class Aggr(RelExpr):
    ...
    def query(self):
        return 'SELECT {}{}{} FROM {}{}{}'\
               .format(', '.join(attr.sql(self) for attr in self.groupbys),
                       (', ' if len(self.groupbys) > 0 else ''),
...
class LeftJoin(RelExpr):
    def __init__(self, left, right):
        assert isinstance(left, RelExpr)
        assert isinstance(right, RelExpr)
        super(LeftJoin, self).__init__([left, right])
    def __str__(self):
        return '{} {}{} {}'.format(paren(self.inputs[0]),
                                   literal(sym.DIFF), '',
                                   paren(self.inputs[1]))
    def validateSubtree(self, context: StatementContext):
        ...
        for i0, a0 in enumerate(self.inputs[0].type.attrs):
            ...
            matches = [(i1, a1) for i1, a1 in enumerate(self.inputs[1].type.attrs)\
                       if a0.name == a1.name]
            if len(matches) > 1:
                raise self.validationError('ambiguity in left join: {} from the left input'
                                           ' matches multiple attributes on the right'\
                                           .format(a0.str_ref_only()))
                ...
                try:
                    ...
                    raise self.validationError('left join cannot equate {} and {}'\
                                               .format(a0, a1))
                self.pairs.append((i0, i1))
        if len(self.pairs) == 0:
            return self.validationError('{}: no attributes with matching names found;'
                           ' left join degnerates into cross product'.format(self))
        for i0, i1 in self.pairs:
            if any((j0, j1) for j0, j1 in self.pairs if j0 != i0 and j1 == i1):
                raise self.validationError('ambiguity in natural join: {} from the right input'
                                           ' matches multiple attributes on the left'\
        ...
        if query in context.cse.keys():
          ...
          context.cse[query] = (idx, len(attrspecs))
    def info(self):
        yield '{} => {}'.format(symbolic(sym.DIFF), self.type)
        if len(self.pairs) == 0:
            yield '|inferred: cross product with no join condition'
        else:
            for i0, i1 in self.pairs:
    ...
    def query(self):
        ...
        if len(self.pairs) > 0:
            eqs = ['{}.{} = {}.{}'.format(self.inputs[0].type.sql_rel(),
                   ...
                   for i0, i1 in self.pairs]
            for i1, a1 in enumerate(self.inputs[1].type.attrs):
              where = ' ON {} WHERE {}.{} IS NULL'.format(' AND '.join(eqs), right, self.inputs[1].type.sql_attr(i1))
              break
        else:
            where = ' ON TRUE'
        is_diff = True
    ...
    def validate(self, context):
        ...
        if context.views.raw_def(self.view) is None:
            ...
            tmp_context = Context(context.configured, context.db, context.check,
                                  context.views.clone(), context.cmdargs, context.cse)

...
let lup = Hashtbl.create 100000

let rec meval
  n ts db x3 =
  try
    Hashtbl.find lup (n, x3)
    with
      | Not_found -> let res = (match n, ts, db, x3 with
        ...
        ((match lookupa (ccompare_string8, equal_string8) db e
           ...
           | Some a ->
             mapa (fun x ->
               ...
               a),
          MPred (e, tms))
    | n, ts, db, MRel rel -> (replicate (size_list ts) rel, MRel rel)) in
    Hashtbl.add lup (n, x3) res; res;;
